...
const GraphView: React.FC<GraphViewProps> = ({ functions, parameters = {} }) => {
  ...
  useEffect(() => {
    ...
    // We base the scale on the height, showing roughly -10 to 10 units vertically
    ...
    function update(t: d3.ZoomTransform) {
      ...
      // Dynamic Grid Step
      // We want grid lines to be roughly 50-100 pixels apart
      const targetPixels = 60;
      const roughStep = targetPixels / (pixelsPerUnit * t.k);
      
      // Calculate a "nice" step size (1, 2, 5 * 10^n)
      ...
      xAxisG.select(".domain").attr("stroke", "#475569").attr("stroke-width", 2);
      xAxisG.selectAll(".tick text").attr("fill", "#64748b").style("font-size", "10px");
      ...
      yAxisG.select(".domain").attr("stroke", "#475569").attr("stroke-width", 2);
      yAxisG.selectAll(".tick text").attr("fill", "#64748b").style("font-size", "10px");
      ...
      functions.forEach(({ expression, color }) => {
        ...
        let normalizedExpr = expression.toLowerCase()
          .replace(/f\(x\)\s*=/g, '')
          .replace(/y\s*=/g, '')
        ...
        for (let x = xDomain[0]; x <= xDomain[1]; x += dx) {
          try {
            ...
            // Add custom parameters to scope
            ...
            let y;
            try {
              // Add log2 support if not present or ensure it's handled correctly
              y = evaluate(normalizedExpr, scope);
              
              // Handle mathjs complex numbers (e.g. sqrt(-1))
              if (y && typeof y === 'object' && 're' in y) {
                // If it has an imaginary part, we don't plot it on a real graph
  ...
  return (
    <div ref={containerRef} className="w-full h-full bg-white rounded-2xl shadow-inner overflow-hidden border border-slate-200 cursor-crosshair relative">
      <svg ref={svgRef} className="w-full h-full" />
      <div className="absolute bottom-4 right-4 flex flex-col gap-2">
        <button 
          onClick={resetView}
          className="bg-white/90 backdrop-blur p-2.5 rounded-xl shadow-lg border border-slate-200 text-slate-600 hover:bg-white hover:text-indigo-600 transition-all active:scale-95"
          title="Reset View"
        >
